...
public class UserDetailProvider implements UserDetailService {
    ...
    public R save(int uid) {
        return ExecuteUtil.getR(mapper.insert(uid),"初始化用户详情完成");
    }

    @Override
    public R update(UserDetail detail) {
        return ExecuteUtil.getR(mapper.updateById(detail),"个人实名认证完成");
    }
    ...
    public R save(OSSPo po) {
        return ExecuteUtil.getR(ossPoMapper.insert(po),"oss图片成功");
    }
}
